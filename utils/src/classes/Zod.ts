...
> extends TZodSchemaConstructorOptions<
...
> extends BaseClass {
  ...
  constructor(
    options: TZodSchemaConstructorOptions<
      TPFieldDefinition,
      TFieldValidationErrors
    >,
  ) {
  ...
  getCurrentConstructorOptions(): TZodSchemaConstructorOptions<
    TPFieldDefinition,
    TFieldValidationErrors
  > {
    return {
...
> extends ZodSchema<TPStringFieldDefinition> {
  constructor(
    options: TStringZodSchemaConstructorOptions<TPStringFieldDefinition>,
  ) {
    super(options);
  }
  ...
  ): StringZodSchema<TPStringFieldDefinition> {
    return new StringZodSchema({
      ...this.getCurrentConstructorOptions(),
      validationErrors,
    });
  ...
  build() {
    ...
    const transformations: TPStringFieldDefinition["transformations"] =
    ...
    if (validations.isNoSpaces === true)
      schema = schema.regex(/^\S*$/, validationMessages.noSpaces);
    ...
    type SchemaType = (typeof validations)["isEmail"] extends true
      ? ZodEmail
      : ZodString;
    type ReturnSchemaType = (typeof validations)["isRequired"] extends true
      ? SchemaType
      : ZodNullish<SchemaType>;

    return (
      validations.isRequired === true ? schema : schema.nullish()
    ) as ReturnSchemaType;
  }
}
...
> extends ZodSchema<TPNumberFieldDefinition> {
  constructor(
    options: TNumberZodSchemaConstructorOptions<TPNumberFieldDefinition>,
  ) {
    super(options);
  }
  ...
  ): NumberZodSchema<TPNumberFieldDefinition> {
    return new NumberZodSchema({
      ...this.getCurrentConstructorOptions(),
      validationErrors,
    });
  ...
  build() {
    const validationMessages: TNumberFieldValidationErrors =
    ...
    const validations: TPNumberFieldDefinition["validations"] =
      this._fieldDefinition.validations;
    const transformations: TPNumberFieldDefinition["transformations"] =
      this._fieldDefinition.transformations;
    ...
    let schema: ZodNumber | ZodCoercedNumber =
      transformations.isCoerce === true
        ? z.coerce.number({ error: errorHandler })
        : z.number({ error: errorHandler });
