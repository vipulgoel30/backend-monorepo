...
export class ValidationErrorMsgs {
  ...
  static minLength(field: string, minLength: number, message?: string): string {
    return message ?? formatStr(messages.FIELD.MIN_LENGTH, { field, minLength });
  }

  static maxLength(field: string, maxLength: number, message?: string): string {
    return message ?? formatStr(messages.FIELD.MAX_LENGTH, { field, maxLength });
  }

  static noSpaces(field: string, message?: string): string {
    return message ?? formatStr(messages.FIELD.NO_SPACE, { field });
  }

  static minValue(field: string, minValue: number, message?: string): string {
    return message ?? formatStr(messages.FIELD.MIN_VALUE, { field, minValue });
  }

  static maxValue(field: string, maxValue: number, message?: string): string {
    return message ?? formatStr(messages.FIELD.MAX_VALUE, { field, maxValue });
  }

  static invalidEmail(field: string, message?: string): string {
    return message ?? formatStr(messages.FIELD.INVALID_EMAIL, { field });
  }
}
